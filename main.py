# GUI Display
# GUI + main (calls tree and Graphs)
...
import Graphs  # Graphss.py in repo
...
class PFM(tk.Tk):
    ...
    def __init__(self):
        ...
        self.geometry("1200x800")  # Increased size to fit the Graphs
    ...
    def searchPage(
        ...
        def search():
            ...
            deleteButton = tk.Button(self.deleteFrame, text="Delete Expense", command=deleteFunc,
                                     font=("Times New Roman", 16), bg="midnightblue")
        ...
        fromButton = tk.Button(self.dateFrame, text="Search From:", command=lambda: open_calendar(fromButton, fromDate),
                               bg="midnightblue", font=("Times New Roman", 16))
        fromButton.pack(side="left", pady=10)

        toDate = tk.StringVar(value=None)
        toButton = tk.Button(self.dateFrame, text="Search To:", command=lambda: open_calendar(toButton, toDate),
                             bg="midnightblue", font=("Times New Roman", 16))
    ...
    def viewPage(self):
        ...
        def displayGraphs():
            GraphsChoice = selected_option1.get()
            GraphsGroupChoice = selected_option2.get()

            group = self.trees[GraphsGroupChoice].newSearch(self.trees[GraphsGroupChoice].root,
                                                            self.trees[GraphsGroupChoice].name, None, None, None, None,
                                                            [])
            ...
            if GraphsChoice == "Pie Chart":

                if 'canvas' in globals():
                    canvas.get_tk_widget().destroy()  # Remove previous Graphs if exists
                ...
                Graphs.make_pie_chart(fig, values, names_and_dates, 111, f"{GraphsGroupChoice} Expense Report")

            elif GraphsChoice == "Bar Chart":
                ...
                Graphs.make_bar_Graphs(fig, values, names_and_dates, 111, f"{GraphsGroupChoice} Expense Report")

            elif GraphsChoice == "Timeline Chart":
                ...
                try:
                    ...
                    for item in group:
                        values.append(float(item[2]))
                        dates.append(item[3])
                    Graphs.make_line_Graphs(fig, values, dates, 111, "Timeline Expense Report")
        ...
        enterButton = tk.Button(self, text="Submit Graphs Choice", font=("Times New Roman", 16), command=displayGraphs)
